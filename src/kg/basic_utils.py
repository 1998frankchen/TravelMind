...
def get_dict_items_sorted_by_decreasing_value(_dict):
    # Get list of dictionary values, sort them, and get indices of sorted elements in original list
    # Then reverse indices so that index-corresponding values are arranged in descending order  
    sort_inds = np.flip(np.argsort(list(_dict.values())))
    # Convert dictionary keys to numpy array and index with descending indices to get key array sorted by value in descending order  
    sorted_keys = np.array(list(_dict.keys()))[sort_inds]
    # Convert dictionary values to numpy array and index with descending indices to get value array sorted in descending order  
