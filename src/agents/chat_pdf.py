...
class ChatPDF:
    ...
    def __init__(
        ...
        if model_type == 'ollama':
            ...
            # Before running:
            # 1. Install dashscope python sdk: pip install dashscope
    ...
    def ingest_all(self, pdf_folder_path: str):
        ...
        self.chain = ({"context":self.retriever, "question":RunnablePassthrough()}  # RunnablePassthrough essentially applies identity mapping to user input query  
                      | self.prompt
                      | self.model
                      | StrOutputParser()
                      )
